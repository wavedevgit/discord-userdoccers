...
| 99999 | CLIENT_ONLY_PENDING | Age verification is pending in the client            |
...
| ~~REQUIRE_CAPTCHA~~                          | ~~The user must complete a reCAPTCHA challenge~~                                                                                            | ~~Complete a reCAPTCHA challenge~~                                                                                                                                                 |
...
<RouteHeader method="GET" url="/users/{user.id}/sessions/{session.id}/activities/{activity.id}/1">
  Get User Activity Join
</RouteHeader>

Returns user activity join.

###### Response Body

| Field    | Type    | Description                       |
| -------- | ------- | --------------------------------- |
| secret   | string  | The activity join secret          |
| join_url | ?string | The URL used to join the activity |

<RouteHeader method="GET" url="/users/@me/custom-themes">
  List Users Me Custom Themes
</RouteHeader>

Returns a 204 empty response on success.

<RouteHeader method="GET" url="/users/@me/gift-intent-dismissals">
  List Gift Intent Dismissals
</RouteHeader>

Returns gift intent dismissals.

###### Response Body

| Field      | Type                                | Description                        |
| ---------- | ----------------------------------- | ---------------------------------- |
| dismissals | array[gift intent dismissal object] | The current gift intent dismissals |

<RouteHeader method="GET" url="/users/@me/billing/wallet/{wallet.id}/information">
  Get Billing Wallet Information
</RouteHeader>

Returns billing wallet information.

###### Query String Params

| Field        | Type    | Description                       |
| ------------ | ------- | --------------------------------- |
| get_history? | boolean | Whether to include wallet history |

<RouteHeader method="POST" url="/users/@me/gift-intents/dismiss">
  Dismiss Gift Intent
</RouteHeader>

Dismisses gift intent.

###### JSON Params

| Field       | Type      | Description          |
| ----------- | --------- | -------------------- |
| intent_type | integer   | The intent type      |
| target_id   | snowflake | The ID of the target |

<RouteHeader method="POST" url="/users/@me/applications/{application.id}/entitlement-ticket">
  Create Entitlement Ticket
</RouteHeader>

Creates an entitlement ticket for an application.

###### JSON Params

| Field     | Type    | Description   |
| --------- | ------- | ------------- |
| test_mode | boolean | The test mode |

<RouteHeader method="POST" url="/users/@me/applications/{application.id}/ticket">
  Create Application Ticket
</RouteHeader>

Creates an application ticket.
...
<RouteHeader method="POST" url="/users/disable-email-notifications">
...
| pixel_uuid | string  | The pixel uuid |
| category   | string  | The category   |
| email_type | integer | The email type |

<RouteHeader method="POST" url="/users/disable-server-highlight-notifications">
  Disable Server Highlight Notifications
</RouteHeader>

Disables server highlight notifications.
