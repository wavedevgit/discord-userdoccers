...
<RouteHeader method="POST" url="/channels/{channel.id}/custom-call-sounds">
...
^1^ The animation ID is a zero-based index in the animation set selected by `animation_type`. `BASIC` only supports ID `0`. For `PREMIUM`, the official client randomly selects IDs `10` through `15`, but any ID from `0` through `20` are supported.
