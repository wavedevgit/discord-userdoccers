...
<RouteHeader method="POST" url="/users/@me/phone/verify">
  Verify Phone Number Without Password
</RouteHeader>

Verifies the code sent to the current user's phone number without requiring the account password.
...
<RouteHeader method="POST" url="/phone-verifications/validate-support-ticket">
  Validate Phone Support Ticket
</RouteHeader>

Validates a phone support ticket.

###### JSON Params

| Field | Type   | Description              |
| ----- | ------ | ------------------------ |
| token | string | The phone support ticket |
